...
class BlackJackSolution:
    ...
    def playWithDealer(self, rounds=100000):
        self.reset()
        self.loadPolicy()
        self.exp_rate = 0
        ...
        for i in range(rounds):
            ...
            if i % 1000 == 0:
                if i != 0:
                    winRate.append(result[0] / i)
                    roundCount.append(i)
                    j += 1

        plt.plot(roundCount, winRate, label='Win Rate')
        plt.xlabel('episode (counted every 1000 episode)')
        plt.ylabel('win rate (calculated every 1000 episode)')
        plt.title('Experiment 2: Win rate vs Episode')
...
if __name__ == "__main__":
    ...
    # play
    result = b.playWithDealer(rounds=500000)
    print(result)